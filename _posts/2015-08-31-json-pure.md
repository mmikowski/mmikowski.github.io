...
follow these conventions.  There isn't an official standard or an Internet Engineering Task Force
project for **JSON-Pure APIs**.  At least not yet ;)

My primary goal with **JSON-Pure APIs** is to provide the best possible user
experience to the end user. Certainly the practices shown below are nothing
new or revolutionary. You will see echoes of SOAP, JSON RPC, JSON API, JSON LD
and Hydra in the recommendations - and probably a few other dozen
standards I somehow missed.

Yet sometimes putting all recommendations in one place provides lots of value.

...
`GET`, `HEAD`, `OPTIONS`, `PATCH`, `POST`, `PUT`, and `TRACE`.  You may be forgiven if
...
the meaning of the response. There are ~38 HTTP response codes.  Below is a complete list.
...
The meaning of any RESTful transaction is therefore at the minimum, the result of
...
A RESTful transaction can be more complex than this for various reasons, some of
which we will discuss below. However, we will ignore the complicating factors
associated with network transport and caching since these issues are universal
to all communication channels.

### RESTful is actually pretty awful
It's time to break the silence and admit what RESTful really is: one of the
...
We will soon review a better way to build internet APIs.  But
before we can fully appreciate that solution, we must first understand the 5 primary
problems with RESTful APIs that make them expensive, tedious, and
error-prone. Let's get started!
...
The intermingling the HTTP protocol of with the meaning of the
...
I've been asked numerous times over the last decade to use a RESTful API instead of JSON-pure.
The last time I *almost* had to support a RESTful API was in 2011.

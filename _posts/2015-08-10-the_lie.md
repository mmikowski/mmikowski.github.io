...
**REST-ful APIs** are gifts bestowed from the heavens by
The One True Web-developer Deity.  REST-ful APIs are everywhere.
Even the marketing folks are pushing them as a positive attribute
in sales material.
...
Roy advocated using the request methods he helped define in the HTTP standards
to impart meaning to HTTP requests.  Thus the following HTTP requests all
have different meanings to the REST faithful:
...
the HTTP request method verb (e.g GET), the request address, 
...
before we can fully appreciate the solution, we must first undertand the 5 primary
problems with REST. Let's get started.
...
The limited vocabulary of HTTP methods and response codes is too vague
...
If this were the only problem, I'd probably be using REST today.
But that is only the first problem, and it gets much uglier as
we shall soon see. Even if we could agree on the meaning of the vocabulary,
we run into another practical problem: client and server HTTP
applications don't support the full REST vocabulary.

### Problem #2: The REST vocabular is not fully supported
Most client and server applicaitons don't support all verbs or response codes for the HTTP
protocol.  For example, most web browsers historically have not support PUT or DELETE -
and still don't when posting a form. And many server applications don't properly
support actions like `DELETE` or `PUT`.

So how do we work around these application limitation? One common method
with browsers is to embed the *intended* verb into a browser form.  Which
means that the REST transaction now includes an *actual* HTTP request method,
a request payload, and *what-we-intended-to-use* request method embedded in
the request payload.
...
across software are `200 OK` and `500 Internal server error`.  After that, support varies
from *pretty good* to *abysmal*.  Because of this, we often see
*what-we-intended-to-use* response codes embedded in the response payload
which is different than the actual HTTP response code.
...
*what-we-really-intended* into the response payload.

There's another problem: we don't have one vocabulary, we have three.
HTTP response codes are numbers (200, 201, 202, etc)
that have no direct correlation with HTTP request methods, which are verbs.
And our payload is typically in JSON. Executing a REST transaction is like
sending a package adressed in Swahilli, with English content, and getting
...
- The request address
- The *what-we-indended-to-use* method embedded in the request payload
- The "real message" in the request payload
- The response code
- The *what-we-indended-to-use* response code embedded in the response payload
- The "real message" in the response payload

Not only do we have two very limited vocabularies that no one agrees upon or
fully supports, but now we have to look in 7 different places to try to fully
...
transmission channel.  Which of course leads us to our next problem.

### Problem #5: REST is tied to HTTP and is not portable
REST breaks one of the fundamental laws about good communications:
the message content should be independent of the transmission method.
This mixing is a time-honored way to confuse those who are communicating.

What's worse, by intermingling the HTTP protocol with the meaning of the
transactions, it makes REST-ful APIs completely non-portable.  Moving a
...
the problems with REST.  In this post, we will refer to this solution as **Pure-JSON APIs**.

### The way forward: Pure-JSON APIs
**Pure-JSON APIs** fix most of the problem we've just discussed.
...
Pure-JSON APIs came about because web developers found that REST-ful APIs
...
REST-ful API. The rest of us can use the Pure-JSON API.

### Parting thoughts:
I've been asked numerous times over the last decade by the REST faithful to use
a REST-ful API instead of a Pure-JSON API.  The last time I *almost* had to support a
REST-ful API was in 2011. Thankfully, the server team agreed to provide an alternate
Pure-JSON API along side the REST-ful API by simply stuffing their *intended* HTTP methods and
response codes into the JSON. After a few months, everyone I knew who used the API had
switched to Pure-JSON. It was just that much better.

When you consider your next API, you might just want to make it REST-less, and
go Pure.

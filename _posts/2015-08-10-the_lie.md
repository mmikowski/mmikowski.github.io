...
what `417 Expectation failed` really means.  Besides Roy.
...
response codes, we run into another practical problem: client and server HTTP
applications rarely completely support REST.

### Problem #2: REST is not fully supported
...
from *pretty good* to *abysmal*.  Because of this, we again often see 
the *what-we-intended-to-use response code* embedded in the response payload
which is different than the actual response code used.
...
limited to express effecively communicate the wide variety of
requests and responses across all applications.
...
**because HTTP is not extensible**. I guess we are back to embedding
...
different vocabularies to complete the transaction. It's like sending a
package adressed in Swahilli, with English content, and getting delivery
confirmation in Japanese.  This complexity is a great source of confusion
...
messages, it makes REST-ful APIs completely non-portable.  Moving a
REST-ful API to some other transmission channel besides HTTP means disentangling 
...
the problems with REST: Pure-JSON APIs.
...
response codes into the JSON. After a few months, everyone I knew who used our API had

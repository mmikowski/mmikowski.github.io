...
# 4. How do we get type safety in JavaScript?
There are a few ways to improve JavaScript type safety. One approach involves
using libraries or frameworks such as [Flow][3] or [TypeScript][4]. However,
these are complex solution that requires transpiling and buying into a ecosystem.
Here we propose a simpler solution that may be especially well-suited for smaller
projects. It only requires 3 steps.
...
can be realized by naming our variables to indicate their type. Most
static type errors become self-evident if we adhere to this convention
as illustrated in the [JS Code standard quick reference][b].
...
formatting and replaced the dangerous `while` loop with a `for` loop.
...
testable, maintainable, compressable, and well documented. Copy this code
instead.
...
known. Rely on the name convention to communicate intended type instead.

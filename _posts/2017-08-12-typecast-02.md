...
```js
// Mismatches become obvious. The following assignment does not work.
let total_str = watch_list / use_bool;

// Problems
// - The return from a divide operation should be a number, not a string
// - The coerced numerical value for a list may not be ase desired
// - Dividing by a boolean will not work
//
...
are patently misleading. We'd rather name our variables sensibly and use the
time saved to focus on new challenges.
...
rewrite our code using the name convention from the 
...
`nodeunit` to crete tests for our type-safe `repeatFn` function. Check out the
test suite for [hi\_score][1] to see how this can be accomplished.

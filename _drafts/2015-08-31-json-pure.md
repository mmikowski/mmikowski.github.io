...
**JSON-Pure APIs** - a name I **made up** simply for for _brevity and convenience_ - 
follow these conventions.  And no, there isn't an official standard 
or an Internet Engineering Task Force project.

My primary with a **JSON-Pure API** is to provide the best possible user
experience through an SPA.  Certainly these practices are nothing new or
revolutionary. You will see echos of SOAP, JSON RPC, JSON API, JSON LD
and Hydra in the recommendations - and probably a few other dozen
standards I somehow missed.

Sometimes putting all recommendations in on place sometimes provids lots of value.
In that spirit, here are my recommendations for **JSON-Pure APIs**:
...
### Use Websockets and JSON
We recommend the WebSockets transport with the JSON data exchange format.

#### WebSockets
The WebSockets transport is generally preferred to HTTP/S.
The transport begins as an HTTP/S request which is then upgraded by
the client and server.  This means it generally passes through firewalls as
well as HTTP/S, yet it provides truly asynchronous, full-duplex, persistent
messaging channel between client and server with very little overhead.
HTTP/S can often include multiple kilobytes of header information thanks
to the misuse of cookies, whereas [WebSockets overhead is measured in
bytes](http://blog.arungupta.me/rest-vs-websocket-comparison-benchmarks/).
As this link shows, WebSockets can provide 4-10x the performance of HTTP.

Prior to WebSockets, the practical requirements for real-time messaging
between a browser and a client was expensive and error-prone. XMPP was
perhaps the prior best choice for real-time messaging, and it was 
certainly painful in comparison.

Are WebSockets always required? Of course not. But the transport is extremely
compelling for SPAs and it's easy to implement.  APIs transported over WebSockets
are **decidedly not RESTful** as there is only a single `send()` method for 
transport, and there are no visible response codes in web server logs.
...
JSON.stringify() to parse and send the data respectively.  The convenience and
correctness of these methods do not have to end at the browser either. 
If we use Node.js as our web server, the very same methods are available.

JSON is also the native format of many minimal-structure databases such as
such as MongoDB or CouchDB.  Even when it's not the native format, Node.js 
...
with little nuance.  Reusing existing language and structure greatly reduces
the cognitive overhead for consumers of the API.  The sections below
illustrates how we can employ well-defined vocabularies for action verbs and
...
at times to it is a good idea to aggregate messages on both the client and
the server using *time-stack* strategy.

#### Schema validation
Once a vocabulary is selected, we need to ensure that messages are valid.
We generally use a [JSON schema](http://json-schema.org/) to validate 
messages.  This isn't hard to implement, and it helps avoid silly and 
...
We want this classic `request-response` approach when the web browser is used
...
successfully transferred or not.  Think of the transport layer as the UPS
...
transaction specific fields.  As a refresher, here it is again:
...
### Make it easy to debug
Using suggested practices can make API debugging APIs more linear compared to
RESTful interfaces.  First, we can ensure message transport has succeeded.
Then we can proceed to inspect the JSON for any and all application-level
meaning.  JSON-schema validation is also very helpful, and we recommend employing a
command-line driven JSON-schema checking utility.

### Make it portable
An application API should be easily portable over any transport method such as
HTTP/S, WebSockets, XMPP, or SMTP. An API developed with the recommendations
shown above should be transport neutral, and should require only minor tweaks
to work over nearly any transport.
